...
import { Eye, Pencil, Star, PowerOff, Power } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import { cn } from "@/lib/utils";
import {
    toggleDashboardProductFeaturedById,
    toggleDashboardProductActiveById,
} from "@/actions/dashboard/products/product";
...
export const DashboardProductCard = ({ product }: IProps) => {
    ...
    const handleToggleDashboardProductFeaturedById = async (id: string) => {
        const response = await toggleDashboardProductFeaturedById(id);
        if (response.success) {
            router.refresh();
        }
    };

    const handleToggleDashboardProductActiveById = async (id: string) => {
        const response = await toggleDashboardProductActiveById(id);
        if (response.success) {
            router.refresh();
        }
    };
            ...
            <div className="relative h-48 w-full overflow-hidden bg-zinc-800">
                ...
                <div className="absolute inset-0 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    <Button
                        ...
                        variant="ghost"
                        className={cn(
                            "h-8 w-8 bg-zinc-900/80 rounded-sm border",
                            isActive
                                ? "border-red-900/40 text-red-500/70 hover:text-red-400 hover:bg-red-500/10"
                                : "border-emerald-900/40 text-emerald-500/70 hover:text-emerald-400 hover:bg-emerald-500/10",
                        )}
                        onClick={() =>
                            handleToggleDashboardProductActiveById(id)
                        }
                    >
                        {isActive ? (
                    ...
                    <Button
                        ...
                        className={cn(
                            "h-8 w-8 bg-zinc-900/80 rounded-sm border",
                            isFeatured
                                ? "border-amber-900/40 text-amber-400 hover:bg-amber-500/10"
                                : "border-zinc-700 text-zinc-500 hover:text-amber-400 hover:bg-amber-500/10",
                        )}
                        onClick={() =>
                            handleToggleDashboardProductFeaturedById(id)
                        }
                    >
